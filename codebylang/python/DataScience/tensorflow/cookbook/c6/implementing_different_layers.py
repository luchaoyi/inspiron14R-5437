...
"""
numpy和tensorflow的同名函数功能大致一致.
numpy是直接计算,tensorflow是描述计算图.

a=[1,2,3] #(3,)

np.expand_dims(a,0)
Out[10]: array([[1, 2, 3]]) #(1,3)

np.expand_dims(a,1)     #(3,1)
Out[17]: 
array([[1],
       [2],
       [3]])
...
"""
Note that many of TensorFlow's layer functions are designed to deal with 4D data (4D = [batch size, width, height, channels]). 
For our example data, we have a batch size of 1, a width of 1, a height of 25, and a channel size of 1. 

To expand dimensions, we use the expand_dims() function,  
and to collapse dimensions, we use the squeeze() function. 

we can calculate the output dimensions of convolutional layers from the formula output_size=(W-F+2P)/S+1, where W is the input size, F is the filter size, P is the padding
size, and S is the stride(步长) size.
"""
#--------Convolution--------
