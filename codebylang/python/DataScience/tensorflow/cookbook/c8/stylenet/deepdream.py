...
def render_deepdream(t_obj, img0=img_noise,
    ...
    #算法步骤
    #载入已训练的CNN
    #选择某一层的多个特征(卷积核)，使用这些卷积核的均值代表它们的总体特征来和图片计算差别
    #对输入的octave 平移后在在子区域上计算和卷积核均值的差(差分is梯度),octave是在不同尺度对原图的缩放

    #设卷积核均值特征为A
    #输入图片为B
    #g是特征和输入的差别,可简单视为g=A-B
    #则B+g=B+A-B=A,即输入图片累加加上梯度step*g更解决A 
    #so img += g*(step / (np.abs(g).mean()+1e-7))
