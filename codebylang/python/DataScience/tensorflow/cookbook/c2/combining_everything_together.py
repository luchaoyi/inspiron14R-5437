...
#iris.data存储特征
#iris.target存储了label
#iris.feature_names特征列名



iris = datasets.load_iris()
binary_target = np.array([1. if x==0 else 0. for x in iris.target]) #原来label有三类,0->1,[1,2]->0,#转换为2分类
iris_2d = np.array([[x[2], x[3]] for x in iris.data]) #取两个特征2,3组成数据集
...
mult_A1_x1=tf.matmul(x1_data,A1)
mult_A2_x2=tf.matmul(x2_data,A2)
my_add=tf.add(mult_A1_x1,mult_A2_x2)
my_output=tf.add(my_add,b)
...
ablineValues = []
for i in x:
    ablineValues.append(slope2*i+slope1*i+intercept)
