...
generations = 10000
vocab_size = 10000 #词汇表使用最常见的10000个单词
...
#转换为小写，并删除标点符号。改变变音符号（ä，ë，ï，ö和ü）和eszetts（ß），使德文词汇标准化为字母数字
# Remove punctuation
...
def translation_model(sess, input_vocab_size, output_vocab_size,
    ...
    model = seq2seq_model.Seq2SeqModel(
          ...
          learning_rate,
          lr_decay_rate,
          forward_only=forward_only, #训练时False,测试时True
          dtype=tf.float32)
    return(model)
