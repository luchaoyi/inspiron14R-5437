...
char_list = list(s_text) #将字符串自动转换为字符列表
...
class LSTM_Model():
    def __init__(self, rnn_size, num_layers, batch_size, learning_rate,
        ...
        #此函数是包装在lstm_cell的Dropout方法，可控制输入，输出和状态流转的dropout
        #self.lstm_cell=tf.nn.rnn_cell.DropoutWrapper(cell, input_keep_prob=1.0,output_keep_prob=1.0, state_keep_prob=1.0,...) 
        ...
        #防止梯度爆炸的梯度截断
        #通过tf.trainable_variables 可以得到整个模型中所有trainable=True的Variable,得到一个列表，里面存有所有可以进行训练的变量。 
