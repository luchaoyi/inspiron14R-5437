...
#导出的分类准则class = sign(X*M0-X*M1 + (M0*M0 - M1*M1)/2))
#基于两个均值点，用核函数映射到高维空间后没有办法计算坐标，只能计算内积，所以
# x*M0 -->(计算x与数据集向量每一个内积求和)/个数 （与平均向量的内积-->向量内积的平均） 
# (M0*M0 - M1*M1)/2 --> (计算自身向量两两内积/个数 - 计算自身向量两两内积/个数)/2
